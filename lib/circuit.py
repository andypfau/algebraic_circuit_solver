...
class Circuit:
    ...
    def equations(self, ref_node: NodeType = 0) -> tuple[list[Eq],list[Expr]]:
        node_voltages = self._create_node_voltages_dict(ref_node)
    ...
    def _create_node_voltages_dict(self, ref_node_name: NodeType) -> dict[NodeType,ComplexType]:
        ...
        for node in nodes:
            if node == ref_node_name:
                ...
                break
        if ref_node is None:
            raise RuntimeError(f'Reference node "{ref_node_name}" not found')
    ...
    def _generate_node_current_equations(self, node_voltages: dict[NodeType,ComplexType], component_currents: dict[tuple[RefdesType,int],ComplexType]) -> list[Eq]:
        ...
        for equation_node in node_voltages.keys():
            ...
            for component in self._components:
                for i_terminal,component_node in enumerate(component.nodes):
